...
impl TunSocket {
    ...
    pub fn new(name: &str) -> Result<TunSocket, Error> {
        ...
        let fd = match unsafe { socket(PF_SYSTEM, SOCK_DGRAM, SYSPROTO_CONTROL) } {
            -1 => return Err(Error::Socket(io::Error::last_os_error().to_string())),
            fd => fd,
        };
        ...
        if unsafe { ioctl(fd, CTLIOCGINFO, &mut info as *mut ctl_info) } < 0 {
            unsafe { close(fd) };
            return Err(Error::IOCtl(io::Error::last_os_error().to_string()));
        }
    ...
    pub fn set_non_blocking(self) -> Result<TunSocket, Error> {
        match unsafe { fcntl(self.fd, F_GETFL) } {
            -1 => Err(Error::FCntl(io::Error::last_os_error().to_string())),
    ...
    pub fn name(&self) -> Result<String, Error> {
        ...
        if unsafe {
            ...
            || tunnel_name_len == 0
        {
            return Err(Error::GetSockOpt(io::Error::last_os_error().to_string()));
        }
